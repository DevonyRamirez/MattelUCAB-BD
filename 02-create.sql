-- ============================================================
-- SCHEMA COMPLETO - ORDEN CORRECTO DE DEPENDENCIAS
-- Para importar sin errores en Supabase / PostgreSQL
-- ============================================================

SET CONSTRAINTS ALL DEFERRED;

-- ============================================================
-- BLOQUE 1: TABLAS SIN DEPENDENCIAS (CATÁLOGOS)
-- ============================================================
...
create unique index CLASIFICACION_PK on CLASIFICACION (ID_CLASIFICACION);
...
create unique index TIPO_TRANSPORTE_PK on TIPO_TRANSPORTE (ID_TIPO_TRANSPORTE);
...
create unique index ESTATUS_FASE_PRODUCCION_PK on ESTATUS_FASE_PRODUCCION (ID_ESTATUS_FP);
...
create unique index ESTATUS_GANADOR_PK on ESTATUS_GANADOR (ID_ESTATUS_GANADOR);
...
create unique index ESTATUS_MEMBRESIA_PK on ESTATUS_MEMBRESIA (ID_ESTATUS_MEMBRESIA);
...
create unique index ESTATUS_PAGO_MEMBRESIA_PK on ESTATUS_PAGO_MEMBRESIA (ID_ESTATUS_PAGO_MEMBRESIA);
...
create unique index ESTATUS_ORDEN_COMPRA_B2C_PK on ESTATUS_ORDEN_COMPRA_B2C (ID_ESTATUS_B2C);
...
create unique index ESTATUS_SUBASTA_PK on ESTATUS_SUBASTA (ID_ESTATUS_SUBASTA);
...
create unique index ESTATUS_BACKORDER_PK on ESTATUS_BACKORDER (ID_ESTATUS_BACKORDER);
...
create unique index ESTATUS_PRECIO_PK on ESTATUS_PRECIO (ID_ESTATUS_PRECIO);
...
create unique index ESTATUS_INVENTARIO_PK on ESTATUS_INVENTARIO (ID_ESTATUS_INVENTARIO);
...
create unique index ESTATUS_ORDEN_B2B_PK on ESTATUS_ORDEN_B2B (ID_ESTATUS_ORDEN_B2B);
...
create unique index ESTATUS_PUJA_PK on ESTATUS_PUJA (ID_ESTATUS_PUJA);
...
create unique index PRUEBA_PK on PRUEBA (ID_PRUEBA);
...
create unique index SET_REGALO_PK on SET_REGALO (ID_SET_REGALO);
...
create unique index PENALIZACION_PK on PENALIZACION (ID_PENALIZACION);
...
create unique index MEMBRESIA_PK on MEMBRESIA (ID_MEMBRESIA);
...
create unique index BANCO_PK on BANCO (ID_BANCO);
...
create unique index TABULADOR_SALARIAL_PK on TABULADOR_SALARIAL (ID_TABULADOR_SALARIAL);
...
create unique index MATERIAPRIMA_PK on MATERIAPRIMA (ID_MATERIAPRIMA);
...
create unique index COLECCION_PK on COLECCION (ID_COLECCION);
...
create unique index ERA_PK on ERA (ID_ERA);
...
create unique index TIPO_CUERPO_PK on TIPO_CUERPO (ID_TIPO_CUERPO);
...
create unique index COLOR_PK on COLOR (ID_COLOR);
...
create unique index PROFESION_PK on PROFESION (ID_PROFESION);
...
create unique index MONEDA_PK on MONEDA (ID_MONEDA);
...
create unique index HORARIO_PK on HORARIO (ID_HORARIO);
...
create unique index CARACTERISTICA_PK on CARACTERISTICA (ID_CARACTERISTICA);
...
create unique index BENEFICIO_PK on BENEFICIO (ID_BENEFICIO);
...
create unique index DESCUENTO_PK on DESCUENTO (ID_DESCUENTO);
...
create unique index TIPO_CRIPTOMONEDA_PK on TIPO_CRIPTOMONEDA (ID_TIPO_CRIPTOMONEDA);
...
create unique index FASE_PK on FASE (ID_FASE);
...
create unique index COURIER_PK on COURIER (ID_COURIER);

-- ============================================================
-- BLOQUE 2: TABLAS CON AUTOREFERENCIA
-- ============================================================
...
create index DIVIDE_FK on LUGAR (FK_LUGAR);
...
create index SE_DIVIDE_FK on CATEGORIA (FK_CATEGORIA_RECURSIVA);

-- ============================================================
-- BLOQUE 3: TABLAS QUE DEPENDEN DE BANCO
-- ============================================================
...
create unique index CHEQUE_PK on CHEQUE (ID_CHEQUE);
create index ES_EXPEDIDO_FK on CHEQUE (FK_BANCO_CHEQUE);
...
create unique index TRANSFERENCIA_BANCARIA_PK on TRANSFERENCIA_BANCARIA (ID_TRANSFERENCIA_BANCARIA);
create index ES_TRANSFERIDO_FK on TRANSFERENCIA_BANCARIA (FK_BANCO);
...
create unique index TARJETA_DEBITO_PK on TARJETA_DEBITO (ID_TARJETA_DEBITO);
create index ES_EMITIDO_FK on TARJETA_DEBITO (FK_BANCO);
...
create unique index TARJETA_CREDITO_PK on TARJETA_CREDITO (ID_TARJETA_CREDITO);
create index EMITE_FK on TARJETA_CREDITO (FK_BANCO);

-- ============================================================
-- BLOQUE 4: TABLAS QUE DEPENDEN DE LUGAR
-- ============================================================
...
create unique index SEDE_PK on SEDE (ID_SEDE);
create index ESTA_UBICADA_FK on SEDE (FK_LUGAR_SEDE);
...
create unique index PERSONA_JURIDICA_PK on PERSONA_JURIDICA (ID_PERSONA_JURIDICA);
create index DIR_FISCAL_JURIDICA_FK on PERSONA_JURIDICA (FK_LUGAR_DIR_FISCAL);
create index DIR_FISICA_JURIDICA_FK on PERSONA_JURIDICA (FK_LUGAR_DIR_FISICA);
...
create unique index CLIENTE_B2C_PK on CLIENTE_B2C (ID_PERSONA_NATURAL);
create index DIRECCION_FISICA_FK on CLIENTE_B2C (FK_LUGAR);
...
create unique index EMPLEADO_PK on EMPLEADO (ID_PERSONA_NATURAL);
create index DIRECCION_FISICA2_FK on EMPLEADO (FK_LUGAR);

-- ============================================================
-- BLOQUE 5: TABLAS QUE DEPENDEN DE LUGAR + OTRAS
-- ============================================================
...
create unique index TASA_CAMBIO_PK on TASA_CAMBIO (ID_TASA_CAMBIO);
create index SE_JUSTIFICA_FK on TASA_CAMBIO (FK_MONEDA);
...
create unique index CRIPTOMONEDA_PK on CRIPTOMONEDA (ID_CRIPTOMONEDA);
create index PERTENECE_A_LA_CADENA_FK on CRIPTOMONEDA (FK_TIPO_CRIPTOMONEDA);
...
create unique index TRANSPORTE_PK on TRANSPORTE (ID_TRANSPORTE);
create index SE_IDENTIFICA_POR_FK on TRANSPORTE (FK_TIPO_TRANSPORTE);
...
create unique index ROL_PRIVILEGIO_PK on ROL_PRIVILEGIO (ID_ROL_PRIVILEGIO);
create index HACE_USO_FK on ROL_PRIVILEGIO (FK_PRIVILEGIO);
create index USA_FK on ROL_PRIVILEGIO (FK_ROL);
...
create unique index CARGO_PK on CARGO (ID_CARGO);
create index TRABAJA_FK on CARGO (FK_DEPARTAMENTO);
create index ES_ASIGNADO_POR_FK on CARGO (FK_TABULADOR_SALARIAL);

create table CATEGORIA_CATEGORIA (
   ID_CATEGORIA_CATEGORIA SERIAL             not null,
   FK_CATEGORIA_MADRE   INT4                 not null,
   FK_CATEGORIA_PADRE   INT4                 not null,
...
create unique index CATEGORIA_CATEGORIA_PK on CATEGORIA_CATEGORIA (ID_CATEGORIA_CATEGORIA);
create index VINCULA_FK on CATEGORIA_CATEGORIA (FK_CATEGORIA_MADRE);
create index ES_VINCULADO_FK on CATEGORIA_CATEGORIA (FK_CATEGORIA_PADRE);
...
create unique index INVENTARIO_MATERIAPRIMA_PK on INVENTARIO_MATERIAPRIMA (ID_INVENTARIO_MATERIAPRIMA);
create index ALMACENA_FK on INVENTARIO_MATERIAPRIMA (PFK_SEDE);
create index ES_ALMACENADA_FK on INVENTARIO_MATERIAPRIMA (PFK_MATERIAPRIMA);
...
create unique index MANIFIESTO_CARGO_PK on MANIFIESTO_CARGO (ID_MANIFIESTOCARGO);
create index ES_TRANSPORTADA_FK on MANIFIESTO_CARGO (FK_TRANSPORTE);
create index SE_ENTREGA_EN_FK on MANIFIESTO_CARGO (FK_LUGAR_DESTINO);
create index SU_ORIGEN_ES_FK on MANIFIESTO_CARGO (FK_LUGAR_ORIGEN);

-- ============================================================
-- BLOQUE 6: BASE_DISENO Y SUS DERIVADAS
-- ============================================================
...
create unique index BASE_DISENO_PK on BASE_DISENO (ID_BASEDISENO);
create index ES_PARTE_DE2_FK on BASE_DISENO (FK_COLECCION);
create index PERTENECE2_FK on BASE_DISENO (FK_ERA);
create index RELACIONA_FK on BASE_DISENO (FK_BASEDISENO);
create index SE_MOLDEA_FK on BASE_DISENO (FK_TIPO_CUERPO);
create index SU_COLOR_DE_OJOS_FK on BASE_DISENO (FK_COLOR_OJOS);
create index SU_TONO_DE_PIEL_FK on BASE_DISENO (FK_COLOR_TONOPIEL);
...
create unique index BASEDISENO_SETREGALO_PK on BASEDISENO_SETREGALO (ID_BASEDISENO_SETREGALO);
create index SE_AGRUPA_FK on BASEDISENO_SETREGALO (FK_BASEDISENO);
create index AGRUPA_FK on BASEDISENO_SETREGALO (FK_SET_REGALO);
...
create unique index CATEGORIA_BASEDISENO_PK on CATEGORIA_BASEDISENO (ID_CATEGORIA_BASEDISENO);
create index SE_VINCULA_FK on CATEGORIA_BASEDISENO (FK_BASEDISENO);
create index SE_RELACIONA_FK on CATEGORIA_BASEDISENO (FK_CATEGORIA);
...
create unique index BASEDISENO_PRUEBA_PK on BASEDISENO_PRUEBA (ID_BASEDISENO_PRUEBA);
create index ES_PROBADA_FK on BASEDISENO_PRUEBA (FK_BASEDISENO);
create index PRUEBA_FK on BASEDISENO_PRUEBA (FK_PRUEBA);
...
create unique index PROFESION_BASEDISENO_PK on PROFESION_BASEDISENO (ID_PROFESION_BASEDISENO);
create index SE_EJERCE_FK on PROFESION_BASEDISENO (FK_PROFESION);
create index EJERCIDA_FK on PROFESION_BASEDISENO (FK_BASEDISENO);
...
create unique index CLASIFICACION_BASEDISENO_PK on CLASIFICACION_BASEDISENO (ID_CLASIFICACION_BASEDISENO);
create index ES_CLASIFICADA_FK on CLASIFICACION_BASEDISENO (FK_BASEDISENO);
create index CLASIFICA_FK on CLASIFICACION_BASEDISENO (FK_CLASIFICACION);
...
create unique index BASE_DISENO_CONSTRUCCION_PK on BASE_DISENO_CONSTRUCCION (ID_BASEDISENO_CONSTRUCCION);
create index CONFORMA_FK on BASE_DISENO_CONSTRUCCION (PFK_PIEZA);
create index MOLDEARA_FK on BASE_DISENO_CONSTRUCCION (PFK_MOLDE);
create index CONSTRUYE_FK on BASE_DISENO_CONSTRUCCION (PFK_MATERIAPRIMA);
create index SE_CONFORMA_FK on BASE_DISENO_CONSTRUCCION (PFK_BASEDISENO);
...
create unique index CARACTERISTICA_BASEDISENO_PK on CARACTERISTICA_BASEDISENO (ID_CARACTERISTICA_BASEDISENO);
...
create table USUARIO (
   ...
   constraint chk_persona_b2c_b2c_empleado check (
      (FK_PERSONA_JURIDICA IS NOT NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NOT NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NOT NULL)
   )
);
create unique index USUARIO_PK on USUARIO (ID_USUARIO);
create index SE_LE_ASIGNA_FK on USUARIO (FK_ROL);
create index SE_CREA_FK on USUARIO (FK_PERSONA_JURIDICA);
create index CREA2_FK on USUARIO (FK_PERSONA_NATURAL_CLIENTE);
create index CREA_FK on USUARIO (FK_PERSONA_NATURAL_EMPLEADO);
...
create table TELEFONO (
   ...
   constraint chk_telefono_b2b_b2c_empleado check (
      (FK_PERSONA_JURIDICA IS NOT NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NOT NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NOT NULL)
   )
);
create unique index TELEFONO_PK on TELEFONO (ID_TELEFONO);
create index CUENTA_CON_FK on TELEFONO (FK_PERSONA_JURIDICA);
create index ADMINISTRA2_FK on TELEFONO (FK_PERSONA_NATURAL_CLIENTE);
create index ADMINISTRA_FK on TELEFONO (FK_PERSONA_NATURAL_EMPLEADO);
...
create table CORREO (
   ...
   constraint chk_correo_b2b_b2c_empleado check (
      (FK_PERSONA_JURIDICA IS NOT NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NOT NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NULL) OR
      (FK_PERSONA_JURIDICA IS NULL AND FK_PERSONA_NATURAL_CLIENTE IS NULL AND FK_PERSONA_NATURAL_EMPLEADO IS NOT NULL)
   )
);
create unique index CORREO_PK on CORREO (ID_CORREO);
create index COORDINA_FK on CORREO (FK_PERSONA_JURIDICA);
create index POSEE2_FK on CORREO (FK_PERSONA_NATURAL_EMPLEADO);
create index POSEE_FK on CORREO (FK_PERSONA_NATURAL_CLIENTE);
...
create unique index LINEA_CREDITO_PK on LINEA_CREDITO (ID_LINEA_CREDITO);
create index ES_ASIGNADO2_FK on LINEA_CREDITO (FK_PERSONA_JURIDICA);
...
create unique index CONTRATO_PERSONAL_PK on CONTRATO_PERSONAL (ID_CONTRATO);
create index LO_CONTRATAN2_FK on CONTRATO_PERSONAL (FK_EMPLEADO);
...
create table MEMBRESIA_CLIENTE_B2C (
   ...
   CONSTRAINT FK_MEMBRESIA_USUARIO FOREIGN KEY (FK_USUARIO) REFERENCES USUARIO(ID_USUARIO)
);
create unique index MEMBRESIA_CLIENTE_B2C_PK on MEMBRESIA_CLIENTE_B2C (ID_MEMBRESIA_CLIENTE_B2C);
create index ES_ADQUIRIDA_FK on MEMBRESIA_CLIENTE_B2C (FK_MEMBRESIA);
create index COMPRA_FK on MEMBRESIA_CLIENTE_B2C (FK_USUARIO);
...
create table HISTORICO_MEMBRESIA (
   ...
   constraint FK_HISTORICO_MEMBRESIA_ESTATUS FOREIGN KEY (FK_ESTATUS_MEMBRESIA) REFERENCES ESTATUS_MEMBRESIA(ID_ESTATUS_MEMBRESIA),
   constraint FK_HISTORICO_MEMBRESIA_MEMBRESIA FOREIGN KEY (FK_MEMBRESIA_CLIENTE_B2C) REFERENCES MEMBRESIA_CLIENTE_B2C(ID_MEMBRESIA_CLIENTE_B2C)
);
create unique index HISTORICO_MEMBRESIA_PK on HISTORICO_MEMBRESIA (ID_HISTORICO_MEMBRESIA);
create index DESCRIBE_FK on HISTORICO_MEMBRESIA (FK_ESTATUS_MEMBRESIA);
create index ES_REGISTRADA_FK on HISTORICO_MEMBRESIA (FK_MEMBRESIA_CLIENTE_B2C);
...
create unique index PAYPAL_PK on PAYPAL (ID_PAYPAL);
create index SE_USA2_FK on PAYPAL (FK_CORREO);
...
create table USUARIO_METODO_PAGO (
   ...
   constraint chk_usuario_metodopago_TDD_TDC check (
      (FK_TARJETA_CREDITO IS NOT NULL AND FK_TARJETA_DEBITO IS NULL) OR
      (FK_TARJETA_CREDITO IS NULL AND FK_TARJETA_DEBITO IS NOT NULL)
   )
);
create unique index USUARIO_METODO_PAGO_PK on USUARIO_METODO_PAGO (ID_USUARIO_METODO_PAGO);
create index REGISTRA_FK on USUARIO_METODO_PAGO (FK_USUARIO);
create index PERTENECE_A_FK on USUARIO_METODO_PAGO (FK_TARJETA_CREDITO);
create index ES_VALIDADA_FK on USUARIO_METODO_PAGO (FK_TARJETA_DEBITO);

-- ============================================================
-- BLOQUE 8: CONTRATOS Y ESTRUCTURA LABORAL
-- ============================================================
...
create unique index CONTRATO_CARGO_PK on CONTRATO_CARGO (ID_CONTRATO_CARGO);
create index ES_ENCARGADO_FK on CONTRATO_CARGO (FK_CARGO);
create index SE_ENCARGARA_FK on CONTRATO_CARGO (FK_CONTRATO);
...
create unique index CONTRATO_BENEFICIO_PK on CONTRATO_BENEFICIO (ID_CONTRATO_BENEFICIO);
create index GOZA_FK on CONTRATO_BENEFICIO (FK_CONTRATO_PERSONAL);
create index ES_GOZADO_FK on CONTRATO_BENEFICIO (FK_BENEFICIO);
...
create unique index ASISTENCIA_PK on ASISTENCIA (ID_ASISTENCIA);
create index ASISTE_FK on ASISTENCIA (FK_CONTRATO);
...
create unique index CONTRATO_HORARIO_PK on CONTRATO_HORARIO (ID_CONTRATO_HORARIO);
create index CUMPLE_FK on CONTRATO_HORARIO (FK_CONTRATO);
create index SE_ORGANIZA_FK on CONTRATO_HORARIO (FK_HORARIO);

-- ============================================================
-- BLOQUE 9: FASE_DISENO Y FASE_PRODUCCION
-- ============================================================
...
create unique index FASE_DISENO_PK on FASE_DISENO (ID_FASE_DISENO);
create index TIENE_FK on FASE_DISENO (FK_BASEDISENO);
create index SE_PRUEBA_FK on FASE_DISENO (FK_PRUEBA);
create index SE_NECESITAN_FK on FASE_DISENO (FK_CARGO);
create index SE_DISENARA_MEDIANTE_FK on FASE_DISENO (FK_FASE);
...
create unique index FASE_PRODUCCION_PK on FASE_PRODUCCION (ID_FASE_PRODUCCION);
create index SE_CONSTRUIRA_MEDIANTE_FK on FASE_PRODUCCION (FK_FASE_DISENO);
create index TRABAJAN_FK on FASE_PRODUCCION (FK_PERSONA_NATURAL);
...
create unique index HISTORICO_FASE_PRODUCCION_PK on HISTORICO_FASE_PRODUCCION (ID_HISTORICO_FASE_PRODUCCION);
create index HISTORICO_FASE_PRODUCCION_FK on HISTORICO_FASE_PRODUCCION (FK_ESTATUS_FP);
create index RESENA_FK on HISTORICO_FASE_PRODUCCION (FK_FASE_PRODUCCION);

-- ============================================================
-- BLOQUE 10: PRODUCTO Y SUS HISTORICOS
-- ============================================================
...
create table PRODUCTO (
   ...
   CONSTRAINT CK_CONDICION_PRODUCTO CHECK (CONDICION_PRODUCTO IN ('NRFB','MINT','Restoration needed','Aprobo las pruebas de calidad'))
);
create unique index PRODUCTO_PK on PRODUCTO (ID_PRODUCTO);
create index SE_CREARA_FK on PRODUCTO (FK_BASEDISENO_CONSTRUCCION);
create index SE_CATEGORIZA_FK on PRODUCTO (FK_CATEGORIA_BASEDISENO);
create index SE_AGRUPARA_FK on PRODUCTO (FK_BASEDISENO_SETREGALO);
create index SE_CLASIFICA_FK on PRODUCTO (FK_CLASIFICACION_BASEDISENO);
create index EJERCE_FK on PRODUCTO (FK_PROFESION_BASEDISENO);
create index SE_CARACTERIZA_FK on PRODUCTO (FK_CARACTERISTICA_BASEDISENO);
create index SE_DISENA_FK on PRODUCTO (FK_BASEDISENO);
create index SE_CREA_POR_FK on PRODUCTO (FK_FASE_PRODUCCION);
...
create table HISTORICO_PRECIO (
   ...
   constraint CKC_TIPO_PRECIO_HISTORIC check (TIPO_PRECIO in ('DET','MAY','SUB')),
...
create unique index HISTORICO_PRECIO_PK on HISTORICO_PRECIO (ID_HISTORICO_PRECIO);
create index SE_VALUA_FK on HISTORICO_PRECIO (FK_PRODUCTO);
create index ESTA_PAUTADO_FK on HISTORICO_PRECIO (FK_ESTATUS_PRECIO);
...
create unique index HISTORICO_INV_PRODUCTO_PK on HISTORICO_INV_PRODUCTO (ID_HISTORICO_INV_PRODUCTO);
create index HISTORICO_INV_PRODUCTO_FK on HISTORICO_INV_PRODUCTO (FK_ESTATUS_INVENTARIO);
create index SE_CERTIFICA_FK on HISTORICO_INV_PRODUCTO (FK_PRODUCTO);

-- ============================================================
-- BLOQUE 11: INVENTARIO_PRODUCTO (sin FK formales en DDL original)
-- ============================================================
...
create table INVENTARIO_PRODUCTO (
   ...
   constraint FK_INVENTARIO_PRODUCTO_SEDE foreign key (FK_SEDE) references SEDE(ID_SEDE),
   constraint FK_INVENTARIO_PRODUCTO_PRODUCTO foreign key (FK_PRODUCTO) references PRODUCTO(ID_PRODUCTO)
);
create unique index INVENTARIO_PRODUCTO_PK on INVENTARIO_PRODUCTO (ID_INVENTARIO_PRODUCTO);
create index CONTIENE_FK on INVENTARIO_PRODUCTO (FK_SEDE);
create index SE_ALMACENA_FK on INVENTARIO_PRODUCTO (FK_PRODUCTO);

-- ============================================================
-- BLOQUE 12: PAGOS B2B Y ÓRDENES
-- ============================================================
...
create table PAGO_B2B (
   ...
   constraint chk_pago_b2b_transferencia_cheque check (
      (FK_TRANSFERENCIA IS NOT NULL AND FK_CHEQUE IS NULL) OR
      (FK_TRANSFERENCIA IS NULL AND FK_CHEQUE IS NOT NULL)
   )
);
create unique index PAGO_B2B_PK on PAGO_B2B (ID_PAGO_B2B);
create index ES_SALDADO_FK on PAGO_B2B (FK_CHEQUE);
create index ES_ABONADO_FK on PAGO_B2B (FK_TRANSFERENCIA);
create index SE_REALIZA_EL_PAGO2_FK on PAGO_B2B (FK_MONEDA);
...
create unique index ORDEN_COMPRA_B2B_PK on ORDEN_COMPRA_B2B (ID_ORDEN_COMPRA_B2B);
create index COMPRA_MASIVAMENTE_FK on ORDEN_COMPRA_B2B (FK_USUARIO);
create index ES_EL_QUE_VENDE_FK on ORDEN_COMPRA_B2B (FK_CONTRATO_CARGO);
...
create table DISTRIBUCION_PAGO_B2B (
   ...
   constraint FK_DISTRIBUCION_PAGO_B2B_PAGO foreign key (FK_PAGO_B2B) references PAGO_B2B(ID_PAGO_B2B),
   constraint FK_DISTRIBUCION_PAGO_B2B_ORDEN foreign key (FK_ORDEN_COMPRA_B2B) references ORDEN_COMPRA_B2B(ID_ORDEN_COMPRA_B2B)
);
create unique index DISTRIBUCION_PAGO_B2B_PK on DISTRIBUCION_PAGO_B2B (ID_DISTRIBUCION_PAGO_B2B);
create index PAGAN_FK on DISTRIBUCION_PAGO_B2B (FK_ORDEN_COMPRA_B2B);
create index SE_REPARTE_FK on DISTRIBUCION_PAGO_B2B (FK_PAGO_B2B);

-- ============================================================
-- BLOQUE 13: ÓRDENES B2C Y PAGOS
-- ============================================================
...
create unique index ORDEN_COMPRA_B2C_PK on ORDEN_COMPRA_B2C (ID_ORDEN_COMPRA_B2C);
create index QUIERE_COMPRAR_FK on ORDEN_COMPRA_B2C (FK_USUARIO);
create index VENDE_FK on ORDEN_COMPRA_B2C (FK_CONTRATO_CARGO);
...
create unique index META_MENSUAL_PK on META_MENSUAL (ID_META_MENSUAL);
create index TIENE_COMO_META_FK on META_MENSUAL (FK_CONTRATO_CARGO);
...
create unique index PAGO_MEMBRESIA_PK on PAGO_MEMBRESIA (ID_PAGO_MEMBRESIA);
create index ES_REALIZADA1_FK on PAGO_MEMBRESIA (FK_USUARIO_METODO_PAGO);
create index ES_REALIZADA2_FK on PAGO_MEMBRESIA (FK_MEMBRESIA);
create index SE_PAGA_FK on PAGO_MEMBRESIA (FK_ESTATUS_PAGO_MEMBRESIA);
create index ES_CLASIFICADA_POR_FK on PAGO_MEMBRESIA (FK_MONEDA);
...
create table PAGO_B2C (
   ...
   constraint chk_metodopago_b2c_paypal_cripto_tdc_tdd check (
      (FK_PAYPAL IS NOT NULL AND FK_CRIPTOMONEDA IS NULL AND FK_TARJETA_CREDITO IS NULL AND FK_TARJETA_DEBITO IS NULL) OR
      (FK_PAYPAL IS NULL AND FK_CRIPTOMONEDA IS NOT NULL AND FK_TARJETA_CREDITO IS NULL AND FK_TARJETA_DEBITO IS NULL) OR
      (FK_PAYPAL IS NULL AND FK_CRIPTOMONEDA IS NULL AND FK_TARJETA_CREDITO IS NOT NULL AND FK_TARJETA_DEBITO IS NULL) OR
      (FK_PAYPAL IS NULL AND FK_CRIPTOMONEDA IS NULL AND FK_TARJETA_CREDITO IS NULL AND FK_TARJETA_DEBITO IS NOT NULL)
   )
);
create unique index PAGO_B2C_PK on PAGO_B2C (ID_PAGO_B2C);
create index ES_CONCILIADO_FK on PAGO_B2C (FK_ORDEN_COMPRA_B2C);
create index SE_VA_PAGAR_FK on PAGO_B2C (FK_PAYPAL);
create index SE_UTILIZARA_FK on PAGO_B2C (FK_CRIPTOMONEDA);
create index SE_UTILIZA_FK on PAGO_B2C (FK_TARJETA_CREDITO);
create index SE_PAGARA_FK on PAGO_B2C (FK_TARJETA_DEBITO);
create index ES_PAGADO_MEDIANTE2_FK on PAGO_B2C (FK_MONEDA);
...
create unique index DESCUENTO_ORDEN_B2C_PK on DESCUENTO_ORDEN_B2C (ID_DESCUENTO_ORDEN_B2C);
create index ES_DESCONTADO_FK on DESCUENTO_ORDEN_B2C (FK_ORDEN_COMPRA_B2C);
create index SE_DESCUENTA_FK on DESCUENTO_ORDEN_B2C (FK_DESCUENTO);
...
create unique index ENVIO_B2C_PK on ENVIO_B2C (ID_ENVIO_B2C);
create index SERA_ENTREGADO2_FK on ENVIO_B2C (FK_ORDEN_COMPRA_B2C);
create index VA_A_ENTREGAR_FK on ENVIO_B2C (FK_COURIER);
create index DIR_ENVIO_B2C_FK on ENVIO_B2C (FK_LUGAR_DESTINO);
create index DIR_ORIGEN_B2C_FK on ENVIO_B2C (FK_LUGAR_ORIGEN);
...
create unique index HISTORICO_ORDEN_COMPRA_B2C_PK on HISTORICO_ORDEN_COMPRA_B2C (ID_HISTORICO_ORDEN_COMPRA_B2C);
create index SUJETA_A_FK on HISTORICO_ORDEN_COMPRA_B2C (FK_ESTATUS_B2C);
create index CONDICIONADO_FK on HISTORICO_ORDEN_COMPRA_B2C (FK_ORDEN_COMPRA_B2C);
...
create unique index DETALLE_ORDEN_B2C_PK on DETALLE_ORDEN_B2C (ID_DETALLE_ORDEN_B2C);
create index SE_APARTA_FK on DETALLE_ORDEN_B2C (FK_PRODUCTO);
create index ES_DETALLADA_POR_FK on DETALLE_ORDEN_B2C (FK_ORDEN_COMPRA_B2C);

-- ============================================================
-- BLOQUE 14: B2B - DETALLES, BACKORDER, PAGOS SUBASTA
-- ============================================================
...
create unique index HISTORICO_ORDEN_COMPRA_B2B_PK on HISTORICO_ORDEN_COMPRA_B2B (ID_HISTORICO_ORDEN_COMPRA_B2B);
create index VA_REGISTRADA_FK on HISTORICO_ORDEN_COMPRA_B2B (FK_ORDEN_COMPRA_B2B);
create index VA_DESCRITA_FK on HISTORICO_ORDEN_COMPRA_B2B (FK_ESTATUS_ORDEN_B2B);
...
create unique index DESCUENTO_ORDEN_B2B_PK on DESCUENTO_ORDEN_B2B (ID_DESCUENTO_ORDEN_B2B);
create index SE_LE_DESCUENTA_FK on DESCUENTO_ORDEN_B2B (FK_ORDEN_COMPRA_B2B);
create index DESCUENTA_FK on DESCUENTO_ORDEN_B2B (FK_DESCUENTO);
...
create unique index DETALLE_MANIFIESTO_CARGO_PK on DETALLE_MANIFIESTO_CARGO (ID_DETALLE_MANIFIESTO_CARGO);
create index SE_DESPACHA_EN_FK on DETALLE_MANIFIESTO_CARGO (FK_MANIFIESTOCARGO);
create index SE_DESPACHA_FK on DETALLE_MANIFIESTO_CARGO (FK_ORDEN_COMPRA_B2B);
...
create unique index DETALLE_ORDEN_B2B_PK on DETALLE_ORDEN_B2B (ID_DETALLE_ORDEN_B2B);
create index SE_PIDE_FK on DETALLE_ORDEN_B2B (FK_PRODUCTO);
create index ESTA_DESCRITA_FK on DETALLE_ORDEN_B2B (FK_ORDEN_COMPRA_B2B);
...
create unique index BACKORDER_PK on BACKORDER (ID_BACKORDER);
create index SERA_PAGADA_FK on BACKORDER (FK_DETALLE_ORDEN_B2B);
...
create unique index HISTORICO_BACKORDER_PK on HISTORICO_BACKORDER (ID_HISTORICO_BACKORDER);
create index ES_REGISTRADO_FK on HISTORICO_BACKORDER (FK_ESTATUS_BACKORDER);
create index SE_LE_REGISTRA_FK on HISTORICO_BACKORDER (FK_BACKORDER);

-- ============================================================
-- BLOQUE 15: SUBASTA, PUJA Y GANADOR
-- ============================================================
...
create unique index SUBASTA_PK on SUBASTA (ID_SUBASTA);
create index SE_SUBASTA_FK on SUBASTA (FK_PRODUCTO);
...
create unique index HISTORICO_SUBASTA_PK on HISTORICO_SUBASTA (ID_HISTORICO_SUBASTA);
create index TIENE_UN_ESTATUS_FK on HISTORICO_SUBASTA (FK_ESTATUS_SUBASTA);
create index ES_DETERMINADA_FK on HISTORICO_SUBASTA (FK_SUBASTA);
...
create unique index PUJA_PK on PUJA (ID_PUJA);
create index OFERTAN_FK on PUJA (FK_SUBASTA);
create index OFERTA_FK on PUJA (FK_USUARIO);
create index NECESITA_FK on PUJA (FK_MONEDA);
...
create unique index HISTORICO_EST_PUJA_PK on HISTORICO_EST_PUJA (ID_HISTORICO_EST_PUJA);
create index SE_ADJUNTA_FK on HISTORICO_EST_PUJA (FK_ESTATUS_PUJA);
create index SE_REALIZA_FK on HISTORICO_EST_PUJA (FK_PUJA);
...
create unique index GANADOR_PK on GANADOR (ID_GANADOR);
create index ES_GANADOR2_FK on GANADOR (FK_PUJA);
create index SE_PENALIZA_FK on GANADOR (FK_PENALIZACION);
...
create unique index PAGO_SUBASTA_PK on PAGO_SUBASTA (ID_PAGO_SUBASTA);
create index ES_VALIDA_FK on PAGO_SUBASTA (FK_USUARIO_METODO_PAGO);
create index ES_REALIZADO_EL_PAGO2_FK on PAGO_SUBASTA (FK_MONEDA);
create index TIENE_QUE_PAGAR_FK on PAGO_SUBASTA (FK_GANADOR);
...
create unique index HISTORICO_EST_GANADOR_PK on HISTORICO_EST_GANADOR (ID_HISTORICO_EST_GANADOR);
create index ES_DESCRITA_FK on HISTORICO_EST_GANADOR (FK_ESTATUS_GANADOR);
create index SE_REGISTRARA_FK on HISTORICO_EST_GANADOR (FK_GANADOR);